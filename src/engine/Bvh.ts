...
export class Bvh {
    ...
    constructor(mesh: TriangleMesh) {
        ...
        const root: BvhNode = this.nodes[this.rootIndex];
        root.firstLeft = 0;
    ...
    private subdivide(index: number) {
        ...
        while (i <= j) {
            ...
            if (this.primitives[i].centroid[axis] < splitPos) {
                i++;
            } else {
    ...
    public intersectionBvh(ray: Ray) {
        ...
        while (stackElements > 0) {
            ...
            if (tmin.t < distanceStack[stackElements]) {
